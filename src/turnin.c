 ...
 */
void usage() {
  fprintf(stderr, "Usage: turnin [-h|--help] [-V|--version] assignment@class "
                  "file1 [file2 [...]]\n");
  exit(1);
}
...
void check_assignment(char *s) {
  ...
  for (; *s; s++) {
    if (!((*s == ' ') || (*s == '_') || (*s == '-') || (*s == '/') ||
          ((*s >= '0') && (*s <= '9')) || ((*s >= 'a') && (*s <= 'z')) ||
          ((*s >= 'A') && (*s <= 'Z')))) {
      fprintf(stderr, "turnin: An assignment can include only ascii characters "
                      "in [a-zA-Z0-9 /_-]\n");
      exit(1);
    }
  }
...
char *timestamp(time_t clock) {
  ...
  sprintf(b, "%02d/%02d/%02d %02d:%02d", t.tm_mon + 1, t.tm_mday,
...
void check_submissions_paths() {
  ...
  for (i = 0; i < sizeof(assignment_path_files) / sizeof(char *); i++) {
    submissions_paths =
        (char *)malloc((path_len + MAX_FILENAME_LENGTH) * sizeof(char));
    strncpy(submissions_paths, assignment_path,
            strlen(assignment_path) - 1); // -1 for traling '.'
...
void setup(char *arg) {
  ...
  if (!pwd->pw_name) {
    fprintf(stderr,
            ...
            " 			 Please report this issue to the system "
            "administrators\n",
            user_uid);
    exit(1);
  }
  ...
  *class ++ = '\0';
  ...
  if (fd) {
    while (fgets(buf, sizeof(buf) - 1, fd) == buf) {
      ...
      if (sscanf(buf, "%s %d", keyword, &n) != 2) {
        ...
        if (n < 1) {
          fprintf(stderr, "turnin: maxfiles in the LIMITS file must be a "
        ...
        if (n < 1) {
          fprintf(stderr, "turnin: maxkbytes in the LIMITS file must be a "
                          "non-zero positive value\n"
                          "        Please notify the Instructor or a TA.\n");
          exit(1);
        }
        ...
        if (n < 1) {
          fprintf(stderr, "turnin: maxturnins in the LIMITS file must be a "
                          "non-zero positive value\n"
                          "        Please notify the Instructor or a TA.\n");
          exit(1);
        }
        ...
        if (n < 0 || n > 100) {
          fprintf(stderr, "turnin: daypenalty in the LIMITS file must be a "
                          "positive value - (0 < penalty < 100)\n"
                          "        Please notify the Instructor or a TA.\n");
          exit(1);
        }
        daypenalty = n;
        ...
        if (n < 0 || n > 100) {
          fprintf(stderr, "turnin: weekendpenalty in the LIMITS file must be a "
                          "positive value - (0 < penalty < 100)\n"
                          "        Please notify the Instructor or a TA.\n");
          exit(1);
        }
        ...
        if (sscanf(buf, "%s %14c", keyword, str_date) != 2) {
          ...
          fprintf(stderr, "turnin: duedate in the LIMITS file must be a "
        ...
        if (sscanf(buf, "%s %14c", keyword, str_date) != 2) {
          ...
          fprintf(stderr, "turnin: lockdate in the LIMITS file must be a "
                          "YYYYMMDD HH:MM format\n"
                          "        Please notify the Instructor or a TA.\n");
          exit(1);
        }
  ...
  if (lstat(assignment_path, &stat) != -1) {
    ...
    for (saveturnin = 1; saveturnin <= maxturnins; saveturnin++) {
      ...
      sprintf(assignment_file, "{on_time,late}/%s-%d{,-*}.tgz", user_name,
              saveturnin);
...
int warn_excludedfiles() {
  ...
  for (fp = fileroot; fp; fp = fp->f_link) {
    switch (fp->f_flag) {
    case F_NOTFILE:
      msg = "not a file, directory, or symlink";
      break;
    case F_BINFILE:
      msg = "binary file";
      break;
    case F_TMPFILE:
      ...
      break;
    case F_HIDDEN:
      msg = "hidden file or directory";
      break;
    case F_NOTOWNER:
      msg = "not owned by user";
      break;
    case F_DOTDOT:
      msg = "pathname contained '..'";
      break;
    case F_ROOTED:
      msg = "only relative pathnames allowed";
      break;
    case F_NOEXIST:
      msg = "does not exist";
      break;
    case F_COREFILE:
      msg = "may not turnin core files";
      break;
    case F_PERM:
 ...
 *
 * su:user tar czf - file-list | su:class tee tempfile > /dev/null in
 * assignmentdir
 */
char *tempfile;
...
void maketar() {
  ...
  *tvp = 0;
   ...
   */
  if (penalty) {
    sprintf(assignment_file, "late/%s-%d-%d.tgz", user_name, saveturnin,
            penalty);
  } else {
...
char *getfilelog() {
  ...
  snprintf(log, logl, "turnin %s: %-8s-%3d %s %3d\n", turninversion, user_name,
           saveturnin, t, nfiles + nsymlinks);
...
void checkdue() {
  ...
  penalty += (tm_curr.tm_wday == 0 || tm_curr.tm_wday == 6) ? weekendpenalty
                                                            : daypenalty;
  ...
  fprintf(stderr,
          "\n*** This turn in will get %d%% penalty, due to late turn in, on "
          "the final grade ***\n",
          penalty);
  wanttocontinue();
}
...
int main(int argc, char *argv[]) {
  ...
  if (nsymlinks) {
    fprintf(stderr, "%s %d+%d (files+symlinks) [%dKB] for %s to %s\n",
            "You are about to turnin", nfiles, nsymlinks, nkbytes, assignment,
            class);
  } else if (nfiles) {
